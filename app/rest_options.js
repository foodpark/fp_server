...
function* simplifyDetails(orderDetail) {
  ...
  for (var key in menuItems) {
    menuItems[key].options = getExtraSkuFromOrderDetail(
      itemsData,
      menuItems[key].sku
    );
...
function* calculateDeliveryPickup(unitId, deliveryTime) {
  logger.info("Calculating delivery pickup", {
    fn: "calculateDeliveryPickup",
    user_id: this.passport.user.id,
    role: this.passport.user.role,
    unit_id: unitId,
...
function* beforeSaveOrderHistory() {
  ...
  logger.info("Prepare to save order ", {
    fn: "beforeSaveOrderHistory",
    user_id: this.passport.user.id,
    role: this.passport.user.role,
    order_sys_order_id: this.resteasy.object.order_sys_order_id
  });
  ...
  if (this.resteasy.operation == "create") {
    ...
    if (
      this.passport.user.role != "CUSTOMER" &&
      this.passport.user.role !== "FOODPARKMGR" &&
      this.passport.user.role !== "UNITMGR"
    ) {
    ...
    try {
      ...
      if (this.resteasy.object.customer_id) {
        customer = yield Customer.getSingleCustomer(
          this.resteasy.object.customer_id
        );
        customer = customer[0];

        if (!customer.user_id) throw new Error("Invalid customer id", 422);
      ...
      if (!this.resteasy.object.commission_type) {
        logger.error("No commission type provided for e-commerce system", {
          ...
          user_id: this.passport.user.id,
          role: this.passport.user.role,
          error: "Missing e-commerce commission type (commission_type)"
        });
    ...
    if (!this.resteasy.object.order_sys_order_id) {
      if (this.resteasy.object.context === "prepay") {
        if (!this.resteasy.object.menu_items_data) {
          ...
          yield Prepay.registerGranuoDebit(
            this.resteasy.object.amount,
            this.resteasy.object.company_id,
            this.resteasy.object.customer_id
          );

          this.resteasy.object.amount = Format.formatPrice(
            this.resteasy.object.amount,
            this.resteasy.object.currency
          );
          this.resteasy.object.order_detail = {
            0: {
              title: "Pre-pay Debit",
              quantity: 1
            }
          ...
          var priorBalance = (yield Loyalty.getPointBalance(
            this.resteasy.object.customer_id,
            coId
          ))[0];
          if (!priorBalance) {
            logger.info(
              "creating new loyalty points record for customer " +
                this.resteasy.object.customer_id +
                " at company " +
                coId
            );
            var initBalance = "1";
            var newLoyalty = (yield Loyalty.createNew(
              this.resteasy.object.customer_id,
              coId,
              initBalance
            ))[0];
            ...
            debug(priorBalance);
            logger.info(
              "incrementing loyalty points for customer " +
                this.resteasy.object.customer_id +
                " at company " +
                coId
            );
            ...
            this.resteasy.queries.push(
              this.resteasy.transaction
                .table("loyalty")
                .where("loyalty.id", priorBalance.id)
                .update(incrementedLoyalty)
            );
          }
        ...
        logger.error("No order id provided for e-commerce system", {
          ...
          user_id: this.passport.user.id,
          role: this.passport.user.role,
          error: "Missing e-commerce (order_sys) order id"
        });
    ...
    if (eCommerce) {
      ...
      if (this.resteasy.object.for_delivery) {
        ...
        var addrDetails = "";
        try {
          addrDetails = (yield DeliveryAddress.getSingleAddress(
            this.resteasy.object.delivery_address_id
          ))[0];
        } catch (err) {
        ...
        if (deliveryTime) {
          var pickup = "";
          try {
            pickup = yield calculateDeliveryPickup.call(
              this,
              this.resteasy.object.unit_id,
    ...
    try {
      ...
      yield payload.limitOrderHistPayloadForPut.call(
        this,
        this.resteasy.object
      );
    } catch (err) {
    ...
    if (
      this.resteasy.object.status &&
      typeof this.resteasy.object.status !== "object"
    ) {
      //meaning it's a processed update on the status, no extra work is required
      ...
      try {
        ...
        logger.error("Error getting prior order status", {
          ...
          delivery_time: deliveryTime,
          error: "Error getting prior order status"
        });
        throw err;
      }
      debug(savedStatus);
...
function* afterCreateOrderHistory(orderHistory) {
  ...
  logger.info(
    "Post order creation processing started for order " + orderHistory.id,
    meta
  );
  var unit = "";
  try {
  ...
  debug(orderDetail);
  var custName =
    this.passport.user.first_name +
    " " +
    this.passport.user.last_name.charAt(0);
  var msg = "";
  var title = "";
  ...
  if (orderHistory.for_delivery) {
    ...
    msg = translator.translate(
      lang,
      "orderRequested_deliveryMessage",
      deliverytime,
      custName,
      orderDetail
    );
  ...
  if (unit.apns_id) msgTarget.os = "ios";
  else msgTarget.os = "android";
  ...
  this.resteasy.queries.push(
    this.resteasy.transaction
      .table("order_history")
      .where("order_history.id", orderHistory.id)
      .update(hash)
  );
  logger.info(
    "Post order creation processing completed for order " + orderHistory.id,
    meta
  );
}
...
function* afterUpdateOrderHistory(orderHistory) {
  ...
  meta.lang = lang;
  logger.info(
    "Post order update processing started for order " + orderHistory.id,
    meta
  );
  var deviceId = "";
  var title = "";
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (!orderHistoryStatus[keys[i]]) {
      // notification not yet sent
      var status = keys[i];
      logger.info(
        "Notification not yet sent for status " +
          status +
          " for order " +
          orderHistory.id,
        meta
      ...
      if (status == "order_paid" || status == "pay_fail") {
        ...
        if (status == "order_paid") {
          ...
          orderAccepted = yield afterOrderPaid(
            orderHistory,
            this.passport,
            user,
            customer,
        ...
        debug(
          "...status update from unit. Notify customer " +
            orderHistory.customer_id
        );
        ...
        if (customer.apns_id) msgTarget.os = "ios";
        else msgTarget.os = "android";
      ...
      switch (status) {
        // From Customer
        case "order_paid":
          msgTarget.title = translator.translate(
            lang,
            "payProcessed",
            orderNum
          ); //"Payment Processed - Order #"+ orderNum;
          msgTarget.message = translator.translate(
            lang,
            "payProcessedMessage",
            custName,
            timestamp.now()
        ...
        case "pay_fail":
          ...
          msgTarget.message = translator.translate(
            ...
            custName,
            timestamp.now()
          ); //"custName +" payment failed at "+ timestamp.now();
          break;
        // From Unit
        case "order_declined":
          ...
          msgTarget.message = translator.translate(
            ...
            company.name
          ); //"company.name +" did not accept your order at this time. Please try again some other time.";
          break;
        case "order_accepted":
          ...
          msgTarget.message = translator.translate(
            ...
            timestamp.now()
          ); //"Order accepted at "+ timestamp.now();
          break;
        case "order_in_queue":
          ...
          msgTarget.message = translator.translate(
            ...
            "orderQueuedMessage",
            orderNum
          ); //Your order #"+ orderNum +" is queued for preparation";
          break;
        case "order_cooking":
          ...
          msgTarget.message = translator.translate(
            ...
            "orderCookingMessage",
            orderNum
          ); //"Your order #"+ orderNum +" started cooking";
          break;
        case "order_ready":
          ...
          msgTarget.message = translator.translate(
            ...
            "orderReadyMessage",
            orderNum
          ); //Your order #"+ orderNum +" is ready!";
          break;
        case "order_picked_up":
          msgTarget.title = translator.translate(lang, "orderPickedUp"); //"Order Picked Up";
          msgTarget.message = translator.translate(
            lang,
            "orderPickedUpMessage",
            orderNum
          ); // "Your order #"+ orderNum +" was picked up!";
          break;
        case "no_show":
          ...
          msgTarget.message = translator.translate(
            ...
            "NoShowMessage",
            orderNum
          ); //"Your order #"+ orderNum +" was not picked up! Did you forget?";
          break;
        ...
        case "order_delivered":
          ...
          msgTarget.message = translator.translate(
            ...
            orderNum
          ); //"Your order #"+ orderNum +" was delivered. Thanks again!";
          break;
        default:
          console.log(status);
          if (status !== "cod_payment_option") {
            ...
            throw new Error(
              "Unknown status " + status + " for order #" + orderHistory.id
            );
          } else {
            notify = false;
          }
      ...
      if (notify) {
        ...
        supplemental.payload = yield addCloudPushSupport(
          msgTarget,
          supplemental
        );
        ...
        logger.info(
          msgTarget.to + " notified of order " + orderHistory.id,
          meta
        );
  ...
  if (updated) {
    ...
    this.resteasy.queries.push(
      this.resteasy.transaction
        .table("order_history")
        .where("order_history.id", orderHistory.id)
        .update({ status: orderHistoryStatus })
    );
  }
  logger.info(
    "Post order update processing completed for order " + orderHistory.id,
    meta
  );
}

function* beforeSaveReview() {
  logger.info("Before save Review - check reviewer and answers", {
    fn: "beforeSaveReview"
  });
  ...
  if (firstName == "" && lastName == "") {
    logger.error(
      "Save review rejected because a first name or last name is not defined in user account",
      { fn: "beforeSaveReview", first_name: firstName, last_name: lastName }
    );
    this.throw(
      "Save review rejected because a first name or last name is not defined in user account",
      403
  ...
  if (answersField) {
    ...
    if (answers && answers.length) {
      ...
      logger.info("Review rating not calculated - unexpected answers value", {
        fn: "beforeSaveReview",
        answers: answers
      });
    }
  } else {
    logger.info("Review rating not calculated - answersField is null", {
      fn: "beforeSaveReview"
    });
  }
...
function* beforeSaveCustomer() {
  ...
  if (this.resteasy.operation == "update") {
    ...
    if (username || password) {
      ...
      if (!this.params.id) {
        var noIdErr =
          "No customer id provided. Update operation requires customer id";
        meta.error = noIdErr;
...
function* beforeSaveUnit() {
  ...
  if (this.resteasy.operation == "create") {
    ...
    if (
      this.params.context &&
      (m = this.params.context.match(/companies\/(\d+)$/))
    ) {
      debug("..company " + m[1]);
      companyId = m[1];
    } else {
    ...
    debug(
      "..user exists for username " +
        username +
        "? Yes: user details; No: undefined"
    );
    debug(existingUser);
    if (existingUser) {
      meta.error = "Name must be unique";
      logger.error(
        "Tried to create unit with duplicate name. Name must be unique within Unit/User tables.",
        meta
      );
    ...
    var unitmgr = {
      ...
      username: username,
      password: password,
      country_id: countryId ? countryId : 1
    };
    var user = "";
    try {
    ...
    var currency_symbol = "";
    try {
      var company = (yield Company.getSingleCompany(
        this.resteasy.object.company_id
      ))[0];
      if (company) {
        var currencyObject = (yield this.resteasy
          .knex("countries")
    ...
    if (username || password) {
      ...
      if (!unitId) {
        var noUnitIdErr =
          "No unit id provided. Update operation requires unit id";
      ...
      debug(unit);
      if (
        (username && username != unit.username) ||
        (password && password != unit.password)
      ) {
    ...
    if (this.resteasy.object.territory_id) {
      logger.info(
        "unit territory changed - finding new currency and updating driver territories"
      );
      ...
      var currencyId = "";
      try {
        var territory = (yield Territories.getSingleTerritory(
          this.resteasy.object.territory_id
        ))[0];
        if (territory) {
          var currencyObject = (yield this.resteasy
            .knex("countries")
            .select("currency", "currency_id")
            .where("id", territory.country_id))[0];
          if (currencyObject) {
            currency = currencyObject.currency;
            currencyId = currencyObject.currency_id;
          }
      ...
      try {
        ...
        if (drivers.length > 0) {
          drivers.forEach(function(value) {
            if (value.id !== null) user_ids.push(value.id);
          });
        }
      } catch (err) {
        meta.error = err;
...
function* beforeSaveCompanies() {
  ...
  if (this.resteasy.operation == "update") {
    ...
    if (
      this.resteasy.object.delivery_chg_amount &&
      company.delivery_chg_amount != this.resteasy.object.delivery_chg_amount
    ) {
      ...
      logger.info(
        "Before company updated - update delivery charge amount",
        meta
      );
      var amount = this.resteasy.object.delivery_chg_amount;
      debug(
        "..delivery charge amount has changed to " + amount + ". Updating.."
      );
      // Update moltin
      var item = "";
      ...
      try {
        ...
        meta.error = err;
        logger.error(
          "Unable to update delivery charge in the ordering system",
          meta
        );
        throw new Error(
          "Error updating delivery charge in ordering system",
          422
        );
      }
...
function* beforeSaveDriver() {
  ...
  if (this.resteasy.operation == "create") {
    logger.info(
      "Before driver created - check for valid company context and unit",
      meta
    );
    debug("...create");
    debug("..getting company");
    ...
    if (
      this.params.context &&
      (n = this.params.context.match(/units\/(\d+)$/))
    ) {
      debug("..unit " + n[1]);
      unitId = n[1];
      meta.unit_id = unitId;
    ...
    if (userId) {
      ...
      var driver = {
        role: "DRIVER",
        first_name: this.resteasy.object.name,
        last_name: createUser.last_name,
        username: createUser.username,
        password: createUser.password,
        territory_id: createUser.territory_id,
...
function* beforeSaveLoyaltyRewards() {
  logger.info("Before loyalty rewards saved", {
    fn: "beforeSaveLoyaltyRewards"
  });
  debug("beforeSaveLoyaltyRewards");
  if (this.resteasy.operation == "create") {
    logger.info(
      "Before loyalty rewards created - check for existing company context",
      { fn: "beforeSaveLoyaltyRewards" }
    );
    if (
      this.params.context &&
      ...
      if (exists) {
        logger.error("Company has existing rewards defined", {
          fn: "beforeSaveLoyaltyRewards",
          company_id: coId
        });
        this.throw(
          "Company has existing rewards defined. Use PUT/PATCH to modify.",
          405
        );
      }
      this.resteasy.object.company_id = coId;
      logger.info("Loyalty rewards will be created for company", {
        fn: "beforeSaveLoyaltyRewards",
        company_id: this.resteasy.object.company_id
      });
    } else {
      logger.error("No company context found for loyalty rewards", {
        fn: "beforeSaveLoyaltyRewards",
        error: "No company context found for loyalty rewards"
      });
...
function* beforeSaveUser() {
  logger.info("Before User saved - encrypt password", {
    fn: "beforeSaveUser",
    params: this.params
  });
  ...
  if (password) {
    try {
      ...
      logger.error("Unable to encrypt user password", {
        fn: "beforeSaveUser",
        param_user_id: this.params.id,
        error: err
      });
...
function* afterUpdateUnit(unit) {
  ...
  logger.info(
    "After Unit updated - check for delivery charge amount change",
    meta
  );
  if (this.resteasy.object.delivery_chg_amount) {
    ...
    try {
      (yield this.resteasy
        .knex("companies")
        .update("delivery_chg_amount", chargeAmt)
        .where("id", unit.company_id))[0];
    } catch (err) {
      meta.error = err;
...
function* afterCreateReview(review) {
  logger.info("After review created - add review approval record", {
    fn: "afterCreateReview",
    review: review
  });
  if (!review) {
  ...
  try {
    var reviewApproval = {
      ...
      status: "New",
      updated_at: this.resteasy.knex.fn.now(),
      created_at: this.resteasy.knex.fn.now()
    };

    this.resteasy.queries.push(
      this.resteasy.transaction.table("review_approvals").insert(reviewApproval)
    );
    logger.info("Review approval record created", {
      fn: "afterCreateReview",
      review_id: review.id,
      reviewApproval: reviewApproval
    });
  } catch (err) {
    logger.error("Unable to create review approval record", {
      fn: "afterCreateReview",
      review_id: review.id,
      error: err
    });
...
function* afterCreateLoyaltyUsed(loyaltyUsed) {
  logger.info(
    "afterCreateLoyaltyUsed - subtracting redeemed loyalty points for customer",
    { fn: "afterCreateLoyaltyUsed", loyaltyUsed: loyaltyUsed }
  );
  if (!loyaltyUsed) {
    logger.error("Parameter loyaltyUsed must be defined", {
      fn: "afterCreateLoyaltyUsed",
      error: "Parameter loyaltyUsed must be defined"
    });
    throw new Error("Parameter loyaltyUsed must be defined");
  }
  var priorBalance = (yield Loyalty.getPointBalance(
    loyaltyUsed.customer_id,
    loyaltyUsed.company_id
  ))[0];
  if (!priorBalance) {
    logger.error("Unable to get loyalty point balance for customer", {
      fn: "afterCreateLoyaltyUsed",
      customer_id: loyaltyUsed.customer_id,
      company: loyaltyUsed.company_id,
      error: "Unable to get loyalty point balance for customer"
    });
    throw new Error(
      "Unable to get loyalty point balance for customer " +
        loyaltyUsed.customer_id +
        ", company " +
        loyaltyUsed.company_id
    );
  ...
  try {
    ...
    this.resteasy.queries.push(
      this.resteasy.transaction
        .table("loyalty")
        .where("id", priorBalance.id)
        .update(updateLoyalty)
    );
    debug("afterCreateLoyaltyUsed success");
    logger.info("Loyalty point balance updated", {
      fn: "afterCreateLoyaltyUsed",
      customer_id: loyaltyUsed.customer_id,
      company_id: loyaltyUsed.company_id,
      priorBalanc_id: priorBalance.id,
      updateLoyalty: updateLoyalty
    });
  } catch (err) {
    logger.error("Unable to update loyalty points", {
      fn: "afterCreateLoyaltyUsed",
      priorBalance_id: priorBalance.id,
      updatedLoyalty: updateLoyalty
    });
    throw err;
  }
}
...
function* addCloudPushSupport(target, supp) {
  ...
  elements.icon = "push";
  if (target.os === "ios") var payload = { aps: elements };
  else var payload = { android: elements };
  return payload;
}

function* afterOrderPaid(
  orderHistory,
  passport,
  user,
  customer,
  ...
  meta.lang = lang;
  logger.info(
    "Post order paid processing starter for order " + orderHistory.id,
    meta
  );
  var deviceId = "";
  var title = "";
  ...
  var msgTarget = { order_id: orderHistory.id };
  debug(
    "...status update from unit. Notify customer " + orderHistory.customer_id
  );
  logger.info("Unit order update. Notify customer", meta);
  // get customer device id
  debug("..Customer gcm id is " + customer.gcm_id);
  ...
  if (!msgTarget.gcmId && !msgTarget.fcmId) {
    ...
    logger.error(
      "No fcm/gcm id for " + msgTarget.to + " " + msgTarget.toId,
      fge
    );
  ...
  logger.info("message", msgTarget);
  debug(
    "sending notification to " + msgTarget.to + " (" + msgTarget.toId + ")"
  );
  logger.info(
    "sending notification to " + msgTarget.to + " (" + msgTarget.toId + ")"
  );
  ...
  yield push.notifyOrderUpdated(orderNum, msgTarget);
  logger.info(
    "Post order paid processing completed for order: " + orderHistory.id,
    meta
  );
  return updatedStatus.order_accepted;
}
...
function* afterUpdateReviewApproval(approval) {
  logger.info("Updating review status and company rating", {
    fn: "afterUpdateReviewApproval",
    approval: approval
  });
  debug("begin afterUpdateReviewApproval function");
  if (!approval) {
    logger.error("No Approval provided for review", {
      fn: "afterUpdateReviewApproval",
      error: "No Approval provided for review"
    });
  ...
  try {
    ...
    logger.error("Unable to Update Review Status", {
      fn: "afterUpdateReviewApproval",
      review_id: approval.review_id,
      hash: hash,
      error: err
  ...
  if (approval.status == "Approved") {
    try {
      ...
      if (companyId) {
        var averageRating = (yield Reviews.getAverageRating(
          companyId.company_id,
          approval.review_id
        ))[0];
        if (averageRating) {
          debug(
            "average rating is now " +
              averageRating.avg_rating +
              " for company ID " +
              companyId.company_id
          );
          var updateCompanyRating = {
            calculated_rating: averageRating.avg_rating
          };
          ...
          this.resteasy.queries.push(
            this.resteasy.transaction
              ...
              .update(updateCompanyRating)
          );
        } else {
          logger.info("Unable to get Average Rating for company", {
            fn: "afterUpdateReviewApproval",
            review_id: approval.review_id,
      ...
      logger.info("Updated Average Rating for company", {
        fn: "afterUpdateReviewApproval",
        review_id: approval.review_id,
        companyId: companyId.company_id
      });
    } catch (err) {
      logger.error("Unable to update Average Rating", {
        fn: "afterUpdateReviewApproval",
        review_id: approval.review_id,
        error: err
      });
...
function* afterReadOrderHistory(orderHistory) {
  logger.info("After Read Order History", {
    fn: "afterReadOrderHistory",
    orderHistory: orderHistory
  });
  debug("afterReadOrderHistory");
  if (
    orderHistory &&
    orderHistory.order_sys_order_id &&
    !orderHistory.order_detail
  ) {
    try {
      var moltin_order_items = yield msc.getOrderDetail(
        orderHistory.order_sys_order_id
      );
    ...
    try {
      var updatedOrder = yield OrderHistory.updateOrder(
        orderHistory.id,
        orderDetail
      );
    } catch (err) {
    ...
    if (!orderHistory) {
      logger.error("No Order History found", {
        fn: "afterReadOrderHistory",
        error: "No Order History found"
      });
    } else {
      if (!orderHistory.order_sys_order_id) {
        logger.error("No Order ID exists on the Order History", {
          fn: "afterReadOrderHistory",
          orderHistory: orderHistory,
          error: "No Order ID exists on the Order History"
        });
      }
      if (orderHistory.order_detail) {
        logger.info(
          "Order Detail already exists on the Order History; skipping get",
          { fn: "afterReadOrderHistory", orderHistory: orderHistory }
        );
      }
    }
  }
...
function* validUnitMgr(params, user) {
  logger.info("Validating Unit Manager", {
    fn: "validUnitMgr",
    user: user,
    params: params
  });
  ...
  var compId = "";
  var unitId = "";
  if (params.context && (m = params.context.match(/companies\/(\d+)/)))
    compId = m[1];

  if (params.table == "units") {
    unitId = params.id;
  } else {
    // some other table with unit in the context
    if (params.context && (n = params.context.match(/units\/(\d+)$/)))
      unitId = n[1];
  }
  if (!compId) {
    logger.error("No company id provided for unit", {
      fn: "validUnitMgr",
      user: user,
      params: params,
      error: "No company id provided for unit"
    });
    throw new Error("No company id provided for unit", 422);
  }
  if (!unitId) {
    logger.error("No unit id provided", {
      fn: "validUnitMgr",
      user: user,
      params: params,
  ...
  if (!valid) {
    logger.info("Unit Manager Invalid", {
      fn: "validUnitMgr",
      user: user,
      valid: valid
    });
  ...
  logger.info("Unit Manager Validated", {
    fn: "validUnitMgr",
    user: user,
    valid: valid
  });
...
module.exports = {
  hooks: {
    authorize: function*(operation, object) {
      ...
      if (operation == "create") {
        if (
          this.params.table == "companies" ||
          this.params.table == "roles" ||
          this.params.table == "order_status_audit" ||
          this.params.table == "territories" ||
          this.params.table == "countries"
        ) {
          if (
          ...
          this.params.table == "loyalty_rewards" ||
          this.params.table === "loyalty_packages"
        ) {
          if (
            !this.isAuthenticated() ||
            !this.passport.user ||
            (this.passport.user.role != "OWNER" &&
              this.passport.user.role != "FOODPARKMGR" &&
              this.passport.user.role != "ADMIN")
          ) {
          ...
          if (
            ...
            (this.passport.user.role != "OWNER" &&
              ...
              this.passport.user.role != "FOODPARKMGR" &&
              this.passport.user.role != "UNITMGR" &&
              this.passport.user.role != "HUBMGR")
          ) {
            this.throw(
              "Create Unauthorized - Hub Manager/Unit Manager/Owners/Admin/Food Park Manager only",
              401
            );
          }
          var valid = false;
          ...
          this.params.table == "delivery_addresses" ||
          this.params.table == "favorites" ||
          this.params.table == "reviews" ||
          this.params.table == "order_history" ||
          this.params.table == "loyalty_used"
        ) {
          ...
          if (
            !customer &&
            this.passport.user.role !== "FOODPARKMGR" &&
            this.passport.user.role !== "UNIT_MGR"
          ) {
        ...
        debug(
          "..authorized " + this.passport.user.role + " to update user info"
        );
      } else if (operation == "update" || operation == "delete") {
        if (
          this.params.table == "territories" ||
          this.params.table == "food_parks" ||
          this.params.table == "roles" ||
          this.params.table == "order_status_audit" ||
          this.params.table == "users"
        ) {
          ...
          if (
            ...
            if (this.passport.user.role == "FOODPARKMGR") {
              var unitId = this.params.id;
              var managedUnits = (yield FoodPark.getManagedUnits(
              ...
              if (!valid)
                this.throw(
                  "Update/Delete Unauthorized - FPM does not manage this unit",
                  401
                );
            }
            if (this.passport.user.role == "OWNER") {
              ...
              var valid = (yield Company.verifyOwner(
                coId,
                this.passport.user.id
              ))[0];
              if (!valid) {
          ...
          if (
            ...
            this.throw(
              "Update/Delete Unauthorized - Hub Managers/Unit Managers/Owners/Admin only",
              401
            );
          } else {
            var valid = false;
          ...
          if (
            ...
            this.throw(
              "Update/Delete Unauthorized - Customers/Admins only",
              401
            );
          } else {
            console.log("..authorized");
  ...
  applyContext: function(query) {
    ...
    if (this.resteasy.operation == "index") {
      ...
      if (
        ...
        this.resteasy.table == "order_history" &&
        context &&
        (m = context.match(/companies\/(\d+)/)) &&
        ...
        return query
          .select("*")
          .where("company_id", m[1])
          .andWhere("unit_id", n[1]);
      } else if (
        this.resteasy.table == "drivers" &&
        context &&
        (m = context.match(/companies\/(\d+)/)) &&
        (n = context.match(/units\/(\d+)/))
      ) {
        ...
        return knex("users")
          ...
          .where({ "drivers.company_id": m[1], "drivers.unit_id": n[1] });
        // return query.select('*').where('company_id',m[1]).andWhere('unit_id',n[1]);
      } else if (
        this.resteasy.table == "favorites" ||
        this.resteasy.table == "reviews"
      ) {
        ...
        this.resteasy.table == "food_parks" &&
        context &&
        (m = context.match(/territories\/(\d+)$/))
      ) {
        debug("..territory id " + m[1]);
        return query.select("*").where("territory_id", m[1]);
      } else if (
        this.resteasy.table == "loyalty_rewards" &&
        context &&
        (m = context.match(/companies\/(\d+)$/))
      ) {
        debug("..company id " + m[1]);
        ...
        this.resteasy.table == "delivery_addresses" &&
        context &&
        (m = context.match(/customers\/(\d+)$/))
      ) {
        ...
        this.resteasy.table == "checkins" &&
        context &&
        (m = context.match(/territories\/(\d+)$/i))
      ) {
        return query
          .select(
            knex.raw(
              "((checkins.check_in is null OR checkins.check_in <= current_timestamp) AND (checkins.check_out is null OR checkins.check_out >= current_timestamp)) as is_active"
            )
          )
          .innerJoin("units", "units.id", "checkins.unit_id")
          .where("territory_id", m[1])
          .andWhere("units.is_deleted", false)
          .andWhereRaw("checkins.check_in >= current_date");
      } else if (
        this.resteasy.table == "loyalty" &&
        context &&
        (m = context.match(/companies\/(\d+)/)) &&
        ...
        return query
          .select("*")
          .where("company_id", m[1])
          .andWhere("customer_id", n[1]);
      } else if (
        this.resteasy.table == "users" &&
        context &&
        (m = context.match(/territories\/(\d+)/))
      ) {
        return query.select("*").where("territory_id", m[1]);
      } else if (
        this.resteasy.table == "territories" &&
        context &&
        (m = context.match(/countries\/(\d+)$/))
      ) {
