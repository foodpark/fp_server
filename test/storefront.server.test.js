...
describe('GET /api/v1/mol/companies', function() {
  it('should return all companies in SFEZ db', function(done) {
    ...
    res.body[0].name.should.equal('Thaitanic');
    res.body[0].should.have.property('order_sys_id');
    res.body[0].order_sys_id.should.equal('1293768990807556920');
    done();
    });
  });
});
...
// Test retrieval of option items for an option categories for specific menu item for Company and Category
...
// 10. Test CRUD of an Option Category for a specific Company/Menu Item
